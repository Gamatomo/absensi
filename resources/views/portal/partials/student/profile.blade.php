...
<div class="space-y-6 max-w-3xl">
    <div class="bg-card border border-border rounded-lg p-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('portal.profile.update') }}" class="space-y-4">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div><label class="text-sm text-muted-foreground">Nama</label><input name="name" value="{{ $student['name'] ?? '' }}" class="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"></div>
                <div><label class="text-sm text-muted-foreground">Email</label><input name="email" value="{{ $student['email'] ?? '' }}" class="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"></div>
                <div><label class="text-sm text-muted-foreground">NISN</label><input name="nisn" value="{{ $student['nisn'] ?? '' }}" class="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"></div>
                <div><label class="text-sm text-muted-foreground">Telepon</label><input name="phone" value="{{ $student['phone'] ?? '' }}" class="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"></div>
                <div class="md:col-span-2"><label class="text-sm text-muted-foreground">Alamat</label><textarea name="address" class="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background" rows="2">{{ $student['address'] ?? '' }}</textarea></div>
            </div>
    ...
    <div class="bg-card border border-border rounded-lg p-6 shadow-sm" x-data="faceRegistrationData()">
        ...
        <button @click="openModal" type="button" class="px-4 py-2 bg-primary text-primary-foreground rounded-lg flex items-center gap-2">
            ...
            @if(!empty($student['faceId'])) Perbarui Wajah @else Daftar Wajah @endif
        </button>
